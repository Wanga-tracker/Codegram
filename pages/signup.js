...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Insert into profiles table (only if signUp succeeded)
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex items-center space-x-3">
            ...
            <label className="text-gray-300 text-sm">
              I agree to the{" "}
              <span className="text-[#00FF9F] hover:underline cursor-pointer">Terms & Conditions</span>
            </label>
          </div>

          <input type="hidden" name="referralCode" value={form.referralCode} />

          <button type="submit" disabled={loading} className="w-full bg-[#00FF9F] text-black font-bold py-2 px-4 rounded-lg hover:bg-[#00cc80] transition transform hover:scale-105 shadow-[0_0_20px_#00FF9F]">
