...
export default function BotsPage() {
  ...
  async function handleVote(botId, type) {
    ...
    if (existingVote === type) {
      ...
      if (type === "likes") {
        newLikes++;
        if (existingVote === "dislikes") newDislikes--; // switch vote
      }
      if (type === "dislikes") {
        newDislikes++;
        if (existingVote === "likes") newLikes--; // switch vote
      }
            ...
            {bot.comments && bot.comments.length > 0 && (
              <div className="mt-4 bg-gray-900/60 p-3 rounded-lg">
                ...
                {bot.comments.map((c, i) => (
                  <p key={i} className="text-xs text-gray-300 border-b border-green-500/20 py-1">
                    {c}
                  </p>
                ))}
              </div>
